...
export const Customers: React.FC = () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: CustomerFields }) =>
    ...
    onError: (err: any) => {
      const errMsg = err.response?.data?.detail || 'Failed to update customer profile.';
      showToast(errMsg, 'error');
    },
  });
  ...
  return (
    <div className="space-y-8 relative">
      ...
      {toast && (
        <div className={`
          fixed bottom-6 right-6 z-[100] flex items-center gap-2 px-5 py-3.5 rounded-2xl shadow-xl border max-w-sm animate-fade-in
          ${toast.type === 'success' 
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-foreground tracking-tight">Customers</h1>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 bg-card rounded-2xl border border-red-500/20 animate-fade-in">
          <AlertTriangle className="w-12 h-12 text-red-400 mx-auto mb-4" />
          <h3 className="font-bold text-foreground text-lg">Access Denied / Connection Failure</h3>
        ...
        <div className="text-center py-20 bg-card rounded-2xl border border-border animate-fade-in">
          ...
          <h3 className="font-bold text-foreground text-lg">No Customers Found</h3>
          <p className="text-muted-foreground text-sm font-medium mt-1">
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-border/30">
                {filteredCustomers.map((customer: any) => (
                  <tr key={customer.id} className="group hover:bg-card/40 transition-colors">
      ...
      {deleteConfirmOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-white/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-card border border-border p-6 rounded-2xl max-w-sm w-full shadow-2xl space-y-6">
            <div>
              <h3 className="text-lg font-bold text-foreground">Delete Customer</h3>
              <p className="text-sm text-muted-foreground mt-2">
                Are you sure you want to delete <span className="font-semibold text-foreground">"{deleteConfirmOpen.full_name}"</span>?
